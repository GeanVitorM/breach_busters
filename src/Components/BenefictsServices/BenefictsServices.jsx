...
const BenefictsServices = ({ benefitsData }) => {
  return (
    <div className="bg-[#38347c] text-white py-16" style={{ fontFamily: 'Montserrat, sans-serif' }}>
      <div className="px-8 lg:px-16 m-44 mb-0 mt-0">
        <h2 className="text-2xl font-bold text-center mb-6">Benefícios</h2>
            ...
            {benefitsData.map((benefit, index) => (
              <SwiperSlide key={index}>
                <div className="bg-white text-black p-6 rounded-lg shadow-lg transition transform hover:scale-105 w-72 h-96 mx-auto">
                  ...
                  <img src={benefit.icon} alt={benefit.title} className="w-20 h-20 mx-auto mb-4" />
        ...
        <div className="hidden md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-8">
          {benefitsData.map((benefit, index) => (
            <div key={index} className="bg-white text-black p-4 rounded-lg shadow-lg transition transform hover:scale-105 h-96">
              <h3 className="text-xl font-bold mb-1 text-center">{benefit.title}</h3>
        ...
        <div className="text-center mt-12">
          <a href="#contact" className="bg-gradient-to-r from-purple-400 to-blue-400 text-white font-bold py-3 px-8 rounded-lg">
            BECOME BREACHLESS
          </a>
        </div>
      </div>
